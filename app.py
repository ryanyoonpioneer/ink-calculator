...
def quote_round(pct):
    """Round every non-zero percentage up to the next whole number."""
    if pct <= 0:
        return 0
...
st.image(preview_removed_area(img_rgb, background_mask), caption="Red = removed/no ink", use_container_width=True)
...
if has_cmyk:
    ...
    st.image(preview_mask_only(img_rgb, cmyk_area, background_mask), caption="CMYK/photo area preview", use_container_width=True)
...
with white_col:
    ...
    white_mode = st.radio(
        ...
        ["Only where white is visible", "Under all printed colors/photos"],
        index=1,
        help=(
    ...
    if has_cmyk:
        cmyk_method = st.radio("CMYK breakdown method", ["GCR / UCR print-style CMYK", "Simple RGB-to-CMYK"], index=0)
    else:
...
with preview_col:
    ...
    with p1:
        st.image(preview_mask_only(img_rgb, printed_object_mask, background_mask), caption="Detected artwork", use_container_width=True)

    with p2:
        st.image(preview_white_plate(img_rgb, live_white_mask, background_mask), caption="White plate preview", use_container_width=True)
...
if has_spot:
    ...
    for ink, mask in assigned.items():
        raw_pct = coverage_percent(mask, total_pixels)
        quoted_pct = quote_round(raw_pct)
        combined_spot_mask = combined_spot_mask | mask

        if ink == "White":
            visible_white_mask = visible_white_mask | mask
            display_ink = "Visible White"
        else:
            display_ink = ink

        results.append({"Ink / Plate": display_ink, "Quoted %": quoted_pct, "Actual %": round(raw_pct, 2), "Type": "Visible spot ink"})
...
raw_white_pct = coverage_percent(white_plate_mask, total_pixels)
quoted_white_pct = quote_round(raw_white_pct)

results.append({"Ink / Plate": "White Underprint", "Quoted %": quoted_white_pct, "Actual %": round(raw_white_pct, 2), "Type": white_mode})

if has_cmyk:
    cmyk_results = rgb_to_cmyk_coverage(img_rgb, cmyk_area, total_pixels, cmyk_method)

    for plate, raw_pct in cmyk_results.items():
        result_type = "CMYK Summary" if plate in ["CMYK photo area", "CMYK total laydown"] else f"CMYK Photo Estimate ({cmyk_method})"
        results.append({"Ink / Plate": plate, "Quoted %": quote_round(raw_pct), "Actual %": round(raw_pct, 2), "Type": result_type})
...
st.success(f"Total quoted ink laydown estimate: {int(laydown_rows['Quoted %'].sum())}%")

st.markdown(
    """
**Simple rule**

- **Quoted %** is rounded up to the next whole number.
- Example: 0.3% becomes 1%, 11.14% becomes 12%.
- **Actual %** is shown only for checking.
"""
)

st.subheader("Final previews")

background_full = cv2.resize(background_mask.astype(np.uint8), (img_rgb_original.shape[1], img_rgb_original.shape[0]), interpolation=cv2.INTER_NEAREST).astype(bool)
printed_full = cv2.resize(printed_object_mask.astype(np.uint8), (img_rgb_original.shape[1], img_rgb_original.shape[0]), interpolation=cv2.INTER_NEAREST).astype(bool)
...
with cols[0]:
    st.image(preview_mask_only(img_rgb_original, printed_full, background_full), caption="Detected artwork", use_container_width=True)

with cols[1]:
    st.image(preview_mask_only(img_rgb_original, spot_full, background_full), caption="Visible spot ink area", use_container_width=True)

with cols[2]:
    st.image(preview_white_plate(img_rgb_original, white_full, background_full), caption="Final white plate", use_container_width=True)
...
if has_cmyk:
    ...
    st.image(preview_mask_only(img_rgb_original, cmyk_full, background_full), caption="Final CMYK/photo area", use_container_width=True)
